...
// Честный статус, но без формулировок, которые звучат как отказ.
// Заявку форма принимает круглосуточно (это не привязано к статусу) —
// разница только в том, когда реально перезвонят. Ночная поломка —
// не повод человека отфутболивать: пишем «перезвоним, как только
// сможем», а не жёсткое «завтра с 9:00» — мастер может ответить и
// раньше, если увидит заявку вечером.
export function describeWorkPhase(phase: WorkPhase): WorkStatusText {
  const { closes } = BUSINESS.hoursRange;
  ...
  return {
    isOpen: false,
    label: phase === "before" ? "Ещё не открылись" : "Сейчас не на линии",
    detail: "Заявку можно оставить в любое время — перезвоним, как только сможем",
  };
}

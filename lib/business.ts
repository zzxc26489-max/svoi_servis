...
export const BUSINESS = {
  ...
  primaryAreas: ["Зеленоград", "Ржавки", "Андреевка", "Красногорск"],
  extendedAreas: [
    "Солнечногорск",
    "Химки",
    "Лобня",
  ...
  // Оператор персональных данных (152-ФЗ). Оператором может быть и
  // физлицо — ИП/ООО для этого не обязательны. ФИО и контакт для
  // отзыва согласия обязаны быть публичными, иначе человек не сможет
  // потребовать удалить свой номер. Используются на странице /privacy.
  operator: {
    fullName: "Романычев Дмитрий Вадимович",
  ...
  // должен совпадать с REVIEWS.length (27 августа добавили ещё 7 —
  // стало 22). ratingsCount и value — общая оценка карточки, её
  // отдельно не пересчитать по одним лишь текстам отзывов, обновлять
  ...
  // Срок гарантии — подтверждён владельцем: от 6 месяцев. Держим
  // здесь, а не в текстах секций: до этого на сайте было размытое
  // «гарантия на работы» в пяти местах, и любая правка срока
  // означала бы искать их все руками.
  warranty: {
    months: 6,
    short: "Гарантия от 6 месяцев",
...
/** Русское склонение существительного при числе: 21 оценка, 22 оценки,
    25 оценок. Раньше числа подставлялись в захардкоженное «оценок» —
    при 21 получалось «21 оценок». */
...
export const RATING_FORMS = ["оценка", "оценки", "оценок"] as const;
export const REVIEW_FORMS = ["отзыв", "отзыва", "отзывов"] as const;
...
export const REVIEWS: Review[] = [
  ...
  },
  {
    ...
    tag: "Стиральная машина",
    rating: 5,
    text: "Моя стиральная машина пошла на взлёт при отжиме. Шумела так, что было слышно с улицы... Обратился к мастеру Андрею. Он приехал, продиагностировал и приговорил подшипники. Всё объяснил, всё показал, был вежлив. В цеху машинку разобрали, произвели нужный ремонт, а в бонус специальной химией отмыли всю машинку изнутри, чтобы она была как новая. Фото и видео предоставили. Вот уже машинка работает много времени, и никаких проблем. Спасибо, что есть такие ответственные мастера. Буду обращаться по любым поломкам остальной техники и буду рекомендовать другим!!!",
  },
  {
    author: "Андрей Бугров",
    tag: "Стиральная машина",
    rating: 5,
    text: "Пишу отзыв спустя месяц!!! Спасибо мастеру Дмитрию, спас стиральную машинку, всем рекомендую, парень знает свое дело!",
  },
  {
    author: "Kalinchenko",
    tag: "Холодильник",
    rating: 5,
    text: "Мастер своего дела. Холодильник работает как новый.",
  },
  {
    author: "Станислав Малыш",
    tag: "Холодильник",
    rating: 5,
    text: "Купил холодильник. Ребята оформили доставку. С чем нужно помогли. Гарантию дали.",
  },
  {
    author: "василиса милаш",
    tag: "Кондиционер",
    rating: 5,
    text: "Сломался кондиционер. Мастер Дмитрий быстро нашел проблему, починил, объяснил причину поломки. Хотела бы выразить благодарность за такую работу, очень приятно что в наше время есть такие мастера. Работают на совесть, теперь буду обращаться только к вам.",
  },
  {
    author: "Ирина Петренко",
    tag: "Холодильник",
    rating: 5,
    text: "Холодильник сломался, приехал мастер с большой буквы — Дмитрий, всё толково объяснил, прекрасно провёл диагностику!",
  },
  {
    author: "Николай Ильин",
    rating: 5,
    text: "Ребята рукастые. Причину поломки моментально вычисляют, свою работу вообщем знают на отлично. Горжусь такими мастерами, побольше бы таких честных и справедливых.",
  },
];
...
export const APPLIANCES: ApplianceGroup[] = [
  ...
  },
  {
    title: "Кофемашины",
    icon: "coffee",
    brands: ["DeLonghi", "Philips", "Saeco", "Krups", "Bosch", "Jura", "Nivona"],
    note: "Уточним после заявки",
  },
  {
    title: "Кондиционеры",
    icon: "air",
    brands: ["Samsung", "LG", "Ballu", "Electrolux", "Haier", "Midea", "Hisense"],
    note: "Уточним после заявки",
  },
];
...
export type QaItem = {
  ...
  answer: string;
  category: WorkCategory;
  /** Кто отвечал — реальный мастер или владелец, без выдуманных
      регалий и стажа. Чего не знаем (точный срок ремонта и т.п.) — в
      ответ не пишем, пока владелец не подтвердит цифру. */
  answeredBy: string;
  /** Как показываем автора вопроса. Можно сократить/анонимизировать
      для приватности человека («М. из Ржавок») — сам вопрос при этом
      не редактируем и не выдумываем. */
  askedBy?: string;
};

// Реальные вопросы с телефона + реальные ответы мастеров. Стартовый
// набор ниже — черновик (типичные вопросы по частым поломкам из
// PRICES), владелец должен вычитать и поправить перед тем, как
// считать их реальными обращениями. answeredBy — по той же
// специализации, что и прямые номера в MASTERS, без выдуманных имён
// спрашивающих (askedBy сознательно не заполнен).
export const QA: QaItem[] = [
  {
    category: "Холодильники",
    question: "Холодильник шумит сильнее обычного, но морозит нормально — это опасно?",
    answer:
      "Не всегда. Часто гудит компрессор на старте или подмерзает вентилятор в No Frost — это чинится без остановки холодильника. Хуже, если стук или скрежет появился резко и усиливается: тут лучше не тянуть и вызвать мастера на диагностику, пока не встал компрессор целиком.",
    answeredBy: "Дмитрий",
  },
  {
    category: "Холодильники",
    question: "Под холодильником скопилась вода — что делать до приезда мастера?",
    answer:
      "Отключите от сети и вытрите пол, чтобы не поскользнуться и не залить проводку. Чаще всего дело в забитой дренажной системе — это недорогой ремонт. Если холодильник продолжает морозить нормально, ждать приезда мастера безопасно, просто держите поддон под сливом сухим.",
    answeredBy: "Дмитрий",
  },
  {
    category: "Стиральные машины",
    question: "Стиральная машина не сливает воду — можно ли открыть люк самому?",
    answer:
      "Сначала отключите машину от сети. Люк с блокировкой открывать силой не стоит — можно сломать замок. Если внизу есть сливной фильтр, аккуратно открутите его над тазом: часто дело в засоре именно там. Если воды после этого не стало меньше или бельё осталось мокрым — вызывайте мастера, дело может быть в помпе.",
    answeredBy: "Андрей (Михалыч)",
  },
  {
    category: "Стиральные машины",
    question: "Машина не греет воду — стирает, но бельё остаётся холодным. В чём причина?",
    answer:
      "Чаще всего это ТЭН — либо перегорел, либо оброс накипью и не успевает нагреть за цикл. Реже — датчик температуры. Своими силами тут не диагностировать, нужно вскрывать бак. Оставьте заявку с описанием — назовём стоимость до выезда, диагностика от 500 ₽.",
    answeredBy: "Андрей (Михалыч)",
  },
  {
    category: "Посудомоечные машины",
    question: "Посудомойка не сливает воду и пахнет затхлым — обязательно вызывать мастера?",
    answer:
      "Сначала проверьте и промойте фильтр на дне камеры — это можно сделать самому, инструкция обычно в паспорте машины. Если после чистки фильтра вода всё равно стоит — забита помпа или шланг слива, здесь уже нужен мастер, разбирать насос самостоятельно не советую.",
    answeredBy: "Андрей",
  },
];

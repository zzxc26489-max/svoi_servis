...
На одном сервере уживаются все три проекта, но по-разному:

| Проект | Где | Почему |
|---|---|---|
| «Свой Сервис» | **этот VPS** | нужен живой процесс Node.js — форма шлёт заявки в Telegram |
| «Сатис» | обычный хостинг Beget | статика, достаточно положить файлы в `public_html` |
| Мастерская Веры | обычный хостинг Beget | то же самое |

Занимать VPS статикой смысла нет: на обычном хостинге у тебя свободно
11 сайтов из 12, и там ничего не надо настраивать.
...
Если понадобится поселить сюда ещё один сайт — заводится такой же файл
со своим `server_name`, конфликта не будет.
